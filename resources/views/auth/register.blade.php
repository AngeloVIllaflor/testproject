        ...
        @foreach($purposes as $purpose)
            <div class="mt-2">
                <label for="purpose_{{ $purpose->id }}" class="inline-flex items-center">
                    ...
                    <span class="ml-2 text-gray-700 hidden">{{ $purpose->purpose }}</span>
                </label>
            </div>
        @endforeach
    </div>
...
<script>
    ...
    modal.addEventListener('show.bs.modal', function () {
        ...
        purposesCheckboxes.forEach(function(checkbox) {
            ...
            var purpose = checkbox.nextElementSibling.nextElementSibling.nextElementSibling.textContent;

            var purposeNameItem = document.createElement('div');
            purposeNameItem.classList.add('grid-item');
            purposeNameItem.textContent = purposeName;

            var purposeItem = document.createElement('div');
            purposeItem.classList.add('grid-item');
            purposeItem.textContent = purpose;

            var locationItem = document.createElement('div');
            locationItem.classList.add('grid-item');
            locationItem.textContent = location;

            modalPurposesList.appendChild(purposeNameItem);
